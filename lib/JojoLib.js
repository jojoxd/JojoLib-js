...
var JojoLib = {
	...
	_core: {
		...
		versionCheck: function(){
			JojoLib.out.todo("rewrite version check");
			$(document).ready(function(){
				if(JojoLib.settings.version.versionCheck){
					...
					$.get('http://jojoxd.nl/JojoLib/newestVersion.php', function(data){
						...
						if(Number(version[0]) <= Number(thisVersion[0])){
							str += "Maintainance ";
							if(Number(version[1]) <= Number(thisVersion[1])){
								str += "Minor ";
								if(Number(version[2]) <= Number(thisVersion[2])){
									str += "Major ";
								}
								else{renderChangelog = true;}
							}
							else{renderChangelog = true;}
						}
						else{renderChangelog = true;}
						...
						if(renderChangelog){
							JojoLib.out.log("VersionCheck", "There's a new version available: " + data + ", you are running version " + JojoLib._vars.version);
	...
	twitch: {
		load: function(where){
			if(typeof where !== 'undefined'){
				...
				JojoLib.core.loadLibrary("twitch", JojoLib._core.getSite + JojoLib.settings.jevascriptFolder + "twitch.js");
			}
		},
	},
