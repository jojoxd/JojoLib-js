...
function JojoLib_UndefinedError(name, message){
	this.name = "JojoLib_UndefinedError " + name;
	this.message = message || "no message given";     // Chrome, Firefox etc.
	this.description = message || "no message given"; // IE...
}
JojoLib_UndefinedError.prototype = new Error();
JojoLib_UndefinedError.prototype.constructor = JojoLib_UndefinedError;
...
function TwitchTVPlayer(streamer, where){
	...
	if(streamer == false){
		throw new UndefinedError("JojoXD-1", "Streamer not found in url");
	}
