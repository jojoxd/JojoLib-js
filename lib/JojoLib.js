...
var JojoLib = {
	...
	youtube: {
		/*
		// whatArr = Json = {x: number, y: number ||"auto", UvID: "unique video ID", UpID: "Unique Playlist ID (not neccasary)"}
		player: function(whatArr, where){
			$(document).ready(function(){
				JojoLib.out.debug("1");
				if(whatArr.length = 1){
					JojoLib.out.debug("2.1");
					// the uid is a video
					JojoLib.out.documentAppend(where, '<iframe width="560" height="315" src="http://www.youtube-nocookie.com/embed/' + whatArr[0] + '" frameborder="0" allowfullscreen></iframe>');
					JojoLib.out.debug("3.1");
				}
				else if(whatArr.length > 1){
					// playlist
					JojoLib.out.debug("2.2")
					JojoLib.out.debug("whatarr[0]: " + whatArr[0] + " whatarr[1]" + whatArr[1]);
					JojoLib.out.documentAppend(where, '<iframe width="560" height="315" src="http://www.youtube-nocookie.com/embed/' + whatArr[0] + '?list=' + whatArr[1] + '" frameborder="0" allowfullscreen></iframe>');
					JojoLib.out.debug("3.2");
				}
			});
		},
		*/
		...
		player: function(whatArr, where){
			...
			$(document).ready(function(){
				...
				if(whatArr.autoplay && typeof(whatArr.autoplay) == "boolean" && whatArr.autoplay == true){
					JojoLib.out.debug("YT - autoplay");
					if(whatArr.UpID){
						options[0] = '&autoplay=1';
					}
					else{
						options[0] = '?autoplay=1';
					}
				...
				JojoLib.out.debug("width: " + whatArr.width + " | height: " + whatArr.height + " | UvID: " + whatArr.UvID);
				
				if(whatArr.UpID){
					JojoLib.out.debug("UpID: " + whatArr.UpID);
					// playlist
					JojoLib.out.documentAppend(where, '<iframe width="'+ whatArr.width +'" height="' + whatArr.height + '" src="http://www.youtube-nocookie.com/embed/' + whatArr.UvID + '?list=' + whatArr.UpID + options[0] +'" frameborder="0" allowfullscreen></iframe>');
				}
				else{
					// video
					JojoLib.out.documentAppend(where, '<iframe width="' + whatArr.width + '" height="' + whatArr.height + '" src="http://www.youtube-nocookie.com/embed/' + whatArr.UvID + options[0] +'" frameborder="0" allowfullscreen></iframe>');
				}
			});
		},
	},
